...
function addThread(id, title, content, author, subreddit) {
	...
	db.getConnection(function(err, connection) {
		...
		query += '.threads(thread_id, title, content, original_poster, subreddit) VALUES (';
		query += connection.escape(id) + ', ';
		query += connection.escape(title) + ', ';
		query += connection.escape(content) + ', ';
		query += connection.escape(author) + ', ';
		query += connection.escape(subreddit) + ');';
